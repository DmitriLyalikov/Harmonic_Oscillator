...
class CustomFigCanvas(FigureCanvas, TimedAnimation):
    ...
    def __init__(self):
        ...
        self.data3 = []
        self.data4 = []
        print(matplotlib.__version__) #Verify Working Version, Older may cause issues depending on machine

        #The data
        self.xlim = 200 #Sampling Rate
        self.n = np.linspace(0, self.xlim - 1, self.xlim)  #X points of each Y value, is the same for all as it corresponds to Time
        self.y1 = (self.n * 0.0) #Coordinates
        self.y2 = (self.n * 0.0)
        self.y3 = (self.n * 0.0)
        self.y4 = (self.n * 0.0)
    ...
    def _draw_frame(self, framedata): #Generate new frames(sampling rate = 200)
        margin = 2

        while(len(self.data1) > 0 and len(self.data2) > 0 and len(self.data3) > 0 and len(self.data4) > 0):

            self.y1 = np.roll(self.y1, -1) #Real Time of 4 graphs
